...
func writeRustProject(outputDir, module string, app model.App, skillLang string, runtimeBundle *runtimeconfig.Bundle) error {
	...
	data := templateData{Module: module, Target: "rust", SkillLang: skillLang, App: app, RuntimeBundle: runtimeBundle}
	files := []generatedFile{
		...
		{Path: filepath.Join("src", "runtime_config.rs"), Template: "rust/runtime_config.rs.tmpl", Data: data},
	...
	if hasOAuthLoginSkill {
		files = append(files, generatedFile{Path: filepath.Join("src", "oauth_auth.rs"), Template: "rust/oauth_auth.rs.tmpl", Data: data})
